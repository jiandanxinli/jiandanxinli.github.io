...
##WebViewClient
  ...
  <br>
  <br>
  - **onPageStarted(WebView view, String url, Bitmap favicon)**：该方法在页面开始加载时回调，可以在这个时候设定一个加载的动画，告诉用户程序在等待网络的响应。
  <br>
  <br>
  - **onPageFinished(WebView view, String url)**：该方法在页面加载结束时调用，**理论上**可以在此时关闭加载动画，进行其他操作。___（这里有一坑，后面会进一步介绍）___
  <br>
  <br>
