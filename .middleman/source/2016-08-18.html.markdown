...
## WebView简介
  ...
  从Android 4.4（KitKat）开始，原本基于WebKit的WebView开始基于Chromium内核，这一改动大大提升了WebView组件的性能以及对HTML5,CSS3,JavaScript的支持。不过它的API却没有很大的改动，在兼容低版本的同时只引进了少部分新的API，并不需要你做很大的改动。<br>
  ...
  在WebView中，有几个地方是我们可以使用来定制我们的WebView各种行为的，分别是：**WebChromeClient**、**WebViewClient**、**WebSettings**以及**JavaScriptInterface**。这些我都会在接下来的文章中一一介绍。
<br><br>
##WebView基本使用
...
（几乎所有的set方法都有相应的get方法，这里就只介绍set了）
  ...
  <br>
    缓存模式有四种：
      <br>
      1. **LOAD_DEFAULT**:默认的缓存使用模式。在进行页面前进或后退的操作时，如果缓存可用并未过期就优先使用缓存，否则从网络上加载数据。
      <br>
      2. **LOAD_CACHE_ELSE_NETWORK**:只要缓存可用就使用缓存，哪怕它们已经过期失效。如果缓存不可用就从网络上加载数据。
      <br>
      3. **LOAD_NO_CACHE**:不使用缓存，只从网络加载数据。
      <br>
