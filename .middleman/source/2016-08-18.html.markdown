...
##WebViewClient
  ...
  <br>这个方法有两个地方需要注意：
  <br>
  1.这个方法只在与服务器无法正常连接时调用，类似于服务器返回错误码的那种错误，该方法是不会回调的，因为你已经和服务器正常连接上了（全怪官方文档(︶^︶)）；<br>
  ...
  <br>
  <br>
  - **onReceivedHttpError(WebView view, WebResourceRequest request, WebResourceResponse errorResponse)**：上一个方法曾说到
  <br>
  <br>
